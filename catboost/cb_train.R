...
y_train <- cb_train$covid_vaccination
x_train <- cb_train %>% select(-covid_vaccination)

train_pool <- catboost.load_pool(data = x_train, label = y_train)
test_pool <- catboost.load_pool(data = select(cb_test, -covid_vaccination))

train_fit <- catboost.train(learn_pool = train_pool,
                      params = list(learning_rate = 0.05,
                                    loss_function = "Logloss",
                                    eval_metric = "AUC"))

preds <- catboost.predict(train_fit, test_pool, prediction_type = "Probability")

write_rds(preds, "cb_test_preds.rds")

preds <- read_rds("cb_test_preds.rds")


cb_test %>%
  select(covid_vaccination) %>%
  mutate(pred = 1- preds) %>%
  write_csv("cb_test_roc.csv")
...
table(round(preds), cb_test$covid_vaccination)

cb_test %>%
  select(covid_vaccination) %>%
  mutate(pred = 1- preds) %>%
  roc_auc(as.factor(covid_vaccination), pred) %>%
  View()

cb_test %>%
  select(covid_vaccination) %>%
  mutate(pred = round(preds)) %>%
  conf_mat(as.factor(covid_vaccination), as.factor(pred))
